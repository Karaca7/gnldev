...
- **The fixtures are synthetic.** They measure whether a judge can tell "same job, written
  ...
  the release gate.
- **Twin brains.** The published pairs were generated by a model; a judge from the same family may
  find that phrasing easier than real user text. Read a passing score as a floor on capability, not a
  promise of field accuracy.
