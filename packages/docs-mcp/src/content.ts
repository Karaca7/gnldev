...
// POST /api/agents/starwars/stream  -> SSE (text-delta/tool-call/tool-result/interrupt/done)`,
  },
  {
    ...
    apis: [`runDurable — the durable counterpart of generateText, takes journal+runId`, `durableTool / durableTools — wraps a tool (set) as exactly-once`, `argsHash — detects determinism drift during replay`, `SideEffectRetryBlockedError / RetryLimitExceededError`, `releaseFailedClaim(journal, { toolName, args }) — lets a FAILED cross-run claim be attempted again; refuses a succeeded one, because that record is the exactly-once guarantee`],
...
await runDurable({ runId, journal, model, tools, prompt });`,
  },
  {
    ...
    apis: [`getRunCost(reader, runId) — computes per-run token/USD cost`, `toTraceSpans(reader, runId) — generates an OTel gen_ai-compatible span list`, `effectivePricingTable(journal) — journal __pricing__ (if present) > DEFAULT_PRICING`, `DEFAULT_PRICING — approximate model pricing table ($/1M tokens)`, `effectivePricingTable(journal) — the table a ceiling reads: the journal's __pricing__ document LAYERED over DEFAULT_PRICING`, `gnl pricing list|set|rm|test — edit that document from the CLI; \`test\` shows which entry a model id resolves to, because priceFor matches by longest prefix`, `Studio → Pricing — the same table in the UI, with a cost preview`],
