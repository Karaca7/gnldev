...
describe('M3 studio time-travel', () => {
  it('state endpoint + fork (live tail); read-only fork 501', async () => {
    ...
    expect((await (await call(ro, '/api/capabilities')).json()).fork).toBe(false);
    const ro501 = await call(ro, '/api/runs/o1/fork', {
    ...
    expect((await (await call(app, '/api/capabilities')).json()).fork).toBe(true);

    // GET state @ step 1 → :input seed (user message) + assistant(tool-call)
    const st = await (await call(app, '/api/runs/o1/state?step=1')).json();
    ...
    const fr = await (
      await call(app, '/api/runs/o1/fork', {
