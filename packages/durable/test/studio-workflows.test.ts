...
describe('studio workflows live + run state', () => {
  it('run-stream: emits step + done events (SSE)', async () => {
    const { app } = setup();
    const res = await call(app, '/api/workflows/demo/run-stream', {
  ...
  it('GET /workflows/run/:runId — step outputs from the journal', async () => {
    const { app } = setup();
    const run = await (await call(app, '/api/workflows/demo/run', {
    ...
    const state = (await (await call(app, '/api/workflows/run/wf-demo-x')).json()) as any;
  ...
  it('GET /workflows/:name/runs — persistent run history (listKeys, startedAt desc)', async () => {
    ...
    const post = (runId: string, n: number) =>
      call(app, '/api/workflows/demo/run', {
    ...
    const runs = (await (await call(app, '/api/workflows/demo/runs')).json()) as any[];
  ...
  it('runs: a suspended run shows suspended; completed after resume (sticky _suspend is handled)', async () => {
    ...
    const run = () => call(app, '/api/workflows/flow/run', {
      method: 'POST', headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ input: { x: 1 }, runId: rid }),
    });

    await run(); // tick=1 → suspend at the 'await' step
    let runs = (await (await call(app, '/api/workflows/flow/runs')).json()) as any[];
    ...
    runs = (await (await call(app, '/api/workflows/flow/runs')).json()) as any[];
