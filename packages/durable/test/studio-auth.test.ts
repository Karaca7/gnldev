...
describe('studio admin↔API split', () => {
  it('createStudioApi: viewer read / admin write (role auth)', async () => {
    ...
    expect((await api.request('/runs')).status).toBe(401); // no header → rejected
    expect((await api.request('/runs', { headers: { 'x-role': 'viewer' } })).status).toBe(200); // viewer reads
    ...
    expect((await api.request('/runs/o1/resume', asViewer({ approvals: { 'call-c': true } }))).status).toBe(403); // viewer CANNOT write
    expect(counter.charges).toBe(0);

    const ok = await api.request('/runs/o1/resume', asAdmin({ approvals: { 'call-c': true } }));
  ...
  it('AuthProvider (roleAuth): read gate + /capabilities public + authRequired', async () => {
    ...
    expect((await api.request('/runs')).status).toBe(401); // no header
    expect((await api.request('/runs', { headers: { 'x-role': 'viewer' } })).status).toBe(401); // wrong scheme
    expect((await api.request('/runs', { headers: { authorization: 'Bearer viw' } })).status).toBe(200); // viewer reads

    // /capabilities is now PUBLIC (so the UI can see the auth mode before login) + authRequired flag.
    const capsRes = await api.request('/capabilities');
  ...
  it('createStudioAdmin: apiBase + swagger', async () => {
    ...
    const spec = (await (await admin.request('/openapi.json')).json()) as any;
    expect(spec.servers[0].url).toBe('/studio/api');
    const sw = await admin.request('/swagger');
