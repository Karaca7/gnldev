...
const get = (api: any, p: string, h?: any) => api.request(p, h ? { headers: h } : undefined).then((r: any) => r.json());
...
describe('studio v2 ops endpoints', () => {
  ...
  it('bearerAuth: no token → 401, correct token → 200', async () => {
    ...
    expect((await api.request('/runs')).status).toBe(401);
    expect((await api.request('/runs', { headers: { authorization: 'Bearer s3cret' } })).status).toBe(200);
    expect((await api.request('/runs', { headers: { authorization: 'Bearer wrong' } })).status).toBe(401);
  });
});
