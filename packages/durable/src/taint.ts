...
/**
 * AUDIT A4 (memory-recall half, opt-in `limits.taintScope: 'thread'`): the THREAD-scoped taint key.
...
export async function markRunTainted(
  ...
  // AUDIT A3: the previous code SWALLOWED a write failure. But the common shape is a SINGLE untrusted
  ...
  // AUDIT A4 (opt-in): ALSO claim the thread key so later runs on the same thread inherit the taint at
