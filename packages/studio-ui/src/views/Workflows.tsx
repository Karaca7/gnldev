 ...
 *
 * PACKAGE #4 DEMOTED THIS TO A LEGACY PATH, and the demotion is the point. An engine-derived runId
 * (`run1_<hex>`) carries no name in its text, so this function correctly returns null for one — and
 * "correctly null" is still an operator staring at an opaque hash. The name was never really missing:
 * the run's `:input` records `workflow: <name>`, and the server now passes it through as
 * `item.workflowName`. So the ORDER matters more than the function does: recorded name first, this
 * only for the older `wf-<name>-<ts>` ids that predate the record. Do not add a `run1_` branch here —
 * a hash has nothing to parse, and reaching for text is exactly the habit this package removed.
 */
/**
  ...
  return (
    <div className="rounded-md border border-warning/40 bg-warning/5 p-3">
      <div className="mb-1.5 flex flex-wrap items-center gap-2">
        ...
        {/* The caller's own name for the job. An engine-derived runId is a hash and says nothing to a
            human, so when the run recorded a workKey it is the ONLY readable thing on this row — shown
            next to the id, never instead of it (every button here addresses the run by runId). */}
        {item.workKey && <Badge tone="muted">{t('inboxWorkKeyLabel')} {item.workKey}</Badge>}
