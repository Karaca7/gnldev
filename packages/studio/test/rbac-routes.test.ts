...
describe('@gnl/studio fine-grained routes (RBAC)', () => {
  it('member: runs agents (200) but cannot manage users/budget (403); reads OK', async () => {
    ...
    expect((await app.request('/users', { method: 'POST', headers: JH(member), body: JSON.stringify({ email: 'x@a.co' }) })).status).toBe(403);
    expect((await app.request('/organizations/acme/budget', { method: 'PUT', headers: JH(member), body: JSON.stringify({ tokenLimit: 5 }) })).status).toBe(403);
    expect((await app.request('/runs', { headers: H(member) })).status).toBe(200);
  });

  it('viewer: cannot run agents (403); reads OK', async () => {
    const { app, viewer } = await setup();
    expect((await runReq(app, viewer)).status).toBe(403);
    expect((await app.request('/runs', { headers: H(viewer) })).status).toBe(200);
  });

  it('admin: management succeeds (users 200, budget 200)', async () => {
    const { app, admin } = await setup();
    expect((await app.request('/users', { method: 'POST', headers: JH(admin), body: JSON.stringify({ email: 'new@a.co', roles: ['member'] }) })).status).toBe(200);
    expect((await app.request('/organizations/acme/budget', { method: 'PUT', headers: JH(admin), body: JSON.stringify({ tokenLimit: 5 }) })).status).toBe(200);
  });
  ...
  it('explicit permissions override the role (member-like perms cannot manage budget; [*] can)', async () => {
    ...
    expect((await app.request('/organizations/acme/budget', { method: 'PUT', headers: JH(like), body: JSON.stringify({ tokenLimit: 5 }) })).status).toBe(403);
    expect((await app.request('/organizations/acme/budget', { method: 'PUT', headers: JH(full), body: JSON.stringify({ tokenLimit: 5 }) })).status).toBe(200);
  });
  ...
  it('PATCH /users/:id viewer→member: the SAME token can now run agents', async () => {
    ...
    const patch = await app.request(`/users/${user.id}`, { method: 'PATCH', headers: JH(admin), body: JSON.stringify({ roles: ['member'] }) });
  ...
  it('PATCH assigns explicit permissions; a member cannot PATCH users (403)', async () => {
    ...
    expect((await app.request(`/users/${user.id}`, { method: 'PATCH', headers: JH(member), body: JSON.stringify({ permissions: ['*'] }) })).status).toBe(403);
    // admin can assign explicit permissions
    const r = await app.request(`/users/${user.id}`, { method: 'PATCH', headers: JH(admin), body: JSON.stringify({ permissions: ['*:read', 'agents:run'] }) });
  ...
  it('GET /permissions/catalog: code-defined list + role presets (RBAC on)', async () => {
    const { app, admin } = await setup();
    const cat = await (await app.request('/permissions/catalog', { headers: H(admin) })).json();
    ...
    expect((await app.request('/permissions/catalog', { method: 'POST', headers: JH(admin), body: '{}' })).status).toBe(404);
  });
  ...
  it('GET /permissions/catalog: FREE tier (no RBAC) → disabled/empty', async () => {
    ...
    const cat = await (await app.request('/permissions/catalog', { headers: H('op') })).json();
