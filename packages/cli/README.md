...
> Install: `npm create gnl@latest` scaffolds a fresh project; `pnpm add -D @gnldev/cli` adds the CLI to an existing one. (Or run it from a [repo clone](https://github.com/Karaca7/gnldev): `pnpm install && pnpm -r build`, then `node packages/cli/dist/cli.js <command>`.)
