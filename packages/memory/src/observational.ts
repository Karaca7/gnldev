...
//
// P2-memory (AUDIT-R2) — OM retrieval mode, v1 (scoped): each observation now carries the
...
//
// D4-om (AUDIT-R2 follow-up): the promised vector-indexed retrieval now exists — opt-in via
...
export interface ObservationalMemoryConfig {
  ...
  /**
   * D4-om (AUDIT-R2 follow-up): opt-in vector-indexed OM retrieval — the honest follow-up the
...
export interface Observation {
  ...
  condensed?: boolean;
  /**
   * P2-memory (AUDIT-R2): the source message range this observation was distilled from
...
/**
 * OM recall tool (P2-memory v1 + D4-om follow-up, AUDIT-R2): an AI SDK tool wrapping
