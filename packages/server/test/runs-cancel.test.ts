...
describe('@gnldev/server GET /runs (P0.3 pagination + filters)', () => {
  ...
  it('?status=suspended returns only suspended runs, paged shape {items,nextCursor}', async () => {
    ...
    await api.request('/agents/stall/run', {
      method: 'POST', headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ runId: 'susp-1', prompt: 'hi' }),
    });
    const res = await api.request('/runs?status=suspended');
    ...
    const completed = await (await api.request('/runs?status=completed')).json();
  ...
  it('?limit clamps to [1,1000]; ?cursor round-trips a page', async () => {
    ...
    const p1 = await (await api.request('/runs?limit=2')).json();
    expect(p1.items.length).toBe(2);
    expect(p1.nextCursor).toBeDefined();
    const p2 = await (await api.request(`/runs?limit=2&cursor=${p1.nextCursor}`)).json();
    expect([...p1.items, ...p2.items].map((r: any) => r.runId).sort()).toEqual(['r0', 'r1', 'r2']);
  });

  it('invalid ?status= → 400', async () => {
    const api = createRestApi({ journal: new InMemoryJournal(), agents: { a: { model: mkModel('ok') } } });
    const res = await api.request('/runs?status=bogus');
...
describe('@gnldev/server POST /runs/:id/cancel (P0.3)', () => {
  it('aborts the composed AbortSignal reaching an in-flight stream generation on this instance', async () => {
    ...
    const streamRes = await api.request('/agents/chat/stream', {
      method: 'POST',
    ...
    const cancelRes = await api.request('/runs/never-1/cancel', { method: 'POST' });
    ...
    const again = await api.request('/runs/never-1/cancel', { method: 'POST' });
  ...
  it('unknown run id / a run belonging to a different organization → 404 (no existence leak)', async () => {
    ...
    const unknown = await api.request('/runs/does-not-exist/cancel', { method: 'POST' });
    expect(unknown.status).toBe(404);

    // globex cannot see/cancel acme's run — same 404 shape, no existence leak.
    const crossOrg = await api.request('/runs/r-acme/cancel', { method: 'POST', headers: { 'x-gnl-org': 'globex' } });
    expect(crossOrg.status).toBe(404);

    // acme itself CAN target its own (already-completed, no in-flight controller) run — 200, cancelled:0.
    const ownScope = await api.request('/runs/r-acme/cancel', { method: 'POST', headers: { 'x-gnl-org': 'acme' } });
  ...
  it('a read-only identity is denied (write-permission gate)', async () => {
    ...
    const noAuth = await api.request('/runs/r1/cancel', { method: 'POST' });
    expect(noAuth.status).toBe(403);
    const viewer = await api.request('/runs/r1/cancel', { method: 'POST', headers: { authorization: 'Bearer viw' } });
