...
`npm create gnl <project-name>` — scaffolds a durable agent project. Calls `@gnl/cli`'s
...
The `@gnl/*` packages (durable, server, studio, cli, ...) are not yet published to npm. Because of
...
2. Link the `@gnl/*` packages locally with `pnpm link --global`.

This restriction will go away once the `@gnl/*` packages are published to npm.
